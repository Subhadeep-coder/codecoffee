...
export class StatsService {
  ...
  async getUserStats(userId: string) {
    ...
    const statCard = {
      ...
      stats: {
        ...
        difficultyBreakdown: {
          easy:
            difficultyStats.find((d) => d.difficulty === 'EASY')?.solved || 0,
          medium:
            difficultyStats.find((d) => d.difficulty === 'MEDIUM')?.solved || 0,
          hard:
            difficultyStats.find((d) => d.difficulty === 'HARD')?.solved || 0,
        },
        ...
        contestStats: {
          ...
          averageRank: contestStats._avg.rank
            ? Math.round(contestStats._avg.rank)
            : null,
  ...
  async getUserProgress(userId: string) {
    ...
    const progressData = {
      easy: {
        solved:
          solvedProblems.find((p) => p.difficulty === 'EASY')?.solved || 0,
        total:
          totalProblems.find((p) => p.difficulty === 'EASY')?._count.id || 0,
      },
      medium: {
        solved:
          solvedProblems.find((p) => p.difficulty === 'MEDIUM')?.solved || 0,
        total:
          totalProblems.find((p) => p.difficulty === 'MEDIUM')?._count.id || 0,
      },
      hard: {
        solved:
          solvedProblems.find((p) => p.difficulty === 'HARD')?.solved || 0,
        total:
          totalProblems.find((p) => p.difficulty === 'HARD')?._count.id || 0,
      },
    };
    ...
    Object.keys(progressData).forEach((difficulty) => {
      ...
      data.percentage =
        data.total > 0 ? ((data.solved / data.total) * 100).toFixed(1) : 0;
    });
  ...
  async getAchievementBadges(userId: string) {
    ...
    if (problemsCreated >= 1)
      badges.push({
        name: 'Problem Creator',
        description: 'Created your first problem',
        icon: '🔨',
        earned: true,
      });
    if (problemsCreated >= 5)
      badges.push({
        name: 'Problem Master',
