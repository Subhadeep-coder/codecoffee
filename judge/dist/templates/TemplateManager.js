...
int main() {
    ...
    static getTemplate(language, problemConfig) {
        ...
        if (problemConfig) {
            ...
            Object.keys(problemConfig).forEach((key) => {
                if (!template.placeholders[key]) {
                    const placeholder = `{{${key}}}`;
                    processedTemplate = processedTemplate.replace(new RegExp(placeholder.replace(/[{}]/g, "\\$&"), "g"), problemConfig[key] || "");
                }
            });
        }
        else {
            ...
            Object.keys(template.placeholders).forEach((key) => {
                const placeholder = `{{${key}}}`;
                processedTemplate = processedTemplate.replace(new RegExp(placeholder.replace(/[{}]/g, "\\$&"), "g"), template.placeholders[key]);
            });
        }
    ...
    static addCustomTemplate(language, template) {
        this.templates.set(language.toLowerCase(), template);
    }
}
exports.TemplateManager = TemplateManager;
_a = TemplateManager;
TemplateManager.templates = new Map();
(() => {
    _a.initializeTemplates();
})();
